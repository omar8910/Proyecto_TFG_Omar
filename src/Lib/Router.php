...
class Router {
    ...
    public static function dispatch():void {
        ...
        $action = preg_replace('/proyectoOmarPHPFinal/','',$_SERVER['REQUEST_URI']);
        ...
        if(!empty($match)){
            ...
            $param = $match[0];
            $action=preg_replace('/'.$match[0].'/',':id',$action);//quitamos la primera parte que se repite siempre.
        }
        ...
        if ($fn){
            ...
            // die('Estoy aqui');
            header('Location: /proyectoOmarPHPFinal/errores/');
        }
